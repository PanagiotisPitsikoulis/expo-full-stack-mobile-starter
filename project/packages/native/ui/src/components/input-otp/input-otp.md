...
```tsx
import { InputOTP } from '@pitsi-ui/native';
```
...
```tsx
import { InputOTP, REGEXP_ONLY_CHARS } from '@pitsi-ui/native';
...
```tsx
import { InputOTP, useBottomSheetAwareHandlers } from '@pitsi-ui/native';
...
```tsx
import { InputOTP, Label, Description, type InputOTPRef } from '@pitsi-ui/native';
...
You can find more examples in the [GitHub repository](<https://github.com/pitsiui-inc/@pitsi-ui/native/blob/main/example/src/app/(home)/components/input-otp.tsx>).
