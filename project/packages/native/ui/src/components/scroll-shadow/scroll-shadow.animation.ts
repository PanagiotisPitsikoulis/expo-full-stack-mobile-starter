...
import { SHADOW_EXIT_ANIMATION_DURATION } from "./scroll-shadow";
...
export function useScrollShadowRootAnimation(options: {
  ...
  const bottomOpacityValue: [number, number] = [topOpacityValue[1], topOpacityValue[0]];
  ...
  const topShadowOpacity = useDerivedValue(() => {
    ...
    if (!isEnabled) return withTiming(0, { duration: SHADOW_EXIT_ANIMATION_DURATION });
    if (visibility === "none") return withTiming(0, { duration: SHADOW_EXIT_ANIMATION_DURATION });
