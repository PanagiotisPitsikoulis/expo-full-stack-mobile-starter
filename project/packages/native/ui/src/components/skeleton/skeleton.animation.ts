...
export function useSkeletonShimmerAnimation(options: {
  ...
  const defaultHighlightColor = useMemo(() => {
    return theme === "dark"
      ? colorKit.setAlpha(colorKit.increaseBrightness(themeColorBackground, 10).hex(), 0.1).hex()
      : colorKit.setAlpha(colorKit.decreaseBrightness(themeColorBackground, 10).hex(), 0.75).hex();
