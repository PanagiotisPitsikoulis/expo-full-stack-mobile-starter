 ...
 */
export function formatValue(value: number, formatOptions?: Intl.NumberFormatOptions): string {
  if (formatOptions) {
 ...
 */
export function denormalizeValue(values: number[], wasArray: boolean): number | number[] {
  if (wasArray) {
    return values;
  }
  return values[0] ?? 0;
